...
coactive.driftrates = [3.36, 1.08, 1.08, 0.31]; % HH, HL, LH, LL
parallel.driftrates = [6.61, 1.67, 6.61, 1.67]; % H, L, H, L 
serial.driftrates   = parallel.driftrates; % H, L, H, L 

coactive.stimnames     = {'HH', 'HL', 'LH', 'LL'};
parallel.dim.stimnames = {'targXH', 'targXL', 'targYH', 'targYL'};
...
% %% Convert to diffusion drift rates
% coactive.driftrates   = logit([1-[rateHH, rateHL, rateLH, rateLL], [rateAYH, rateAYL, rateXHB, rateXLB, rateAB, rateABhl, rateABlh, rateABhh]]);
% parallel.driftrates   = logit([1-[rateXH, rateXL, rateYH, rateYL], [rateXH, rateXL, rateYH, rateYL, rateA, rateB, rateAH, rateBH]]); % serial drift rates = parallel drift rates
% serial.driftrates     = logit([1-[rateXH, rateXL, rateYH, rateYL], [rateXH, rateXL, rateYH, rateYL, rateA, rateB, rateAH, rateBH]]); 
% coactive.stimnames   = {'HH', 'HL', 'LH', 'LL', 'AYh', 'AYl', 'XhB', 'XlB', 'AB', 'ABhl', 'ABlh', 'ABhh'};
% parallel.dim.stimnames   = {'targXH', 'targXL', 'targYH', 'targYL', 'Xh', 'Xl', 'Yh', 'Yl', 'A', 'B', 'Ah', 'Bh'};
% serial.dim.stimnames     = parallel.dim.stimnames;
...
coactive.SIC = sic(coactive.S.correct(:, mstrfind(coactive.stimnames, {'LL', 'LH', 'HL', 'HH'})));
...
parallel.cdfs.correct = [parallel.dim.cdfs.correct(:, mstrfind(parallel.dim.stimnames, {'targXH'}))  .*  parallel.dim.cdfs.correct(:, mstrfind(parallel.dim.stimnames, {'targYH'})),... % HH
                         parallel.dim.cdfs.correct(:, mstrfind(parallel.dim.stimnames, {'targXH'}))  .*  parallel.dim.cdfs.correct(:, mstrfind(parallel.dim.stimnames, {'targYL'})),... % HL
                         parallel.dim.cdfs.correct(:, mstrfind(parallel.dim.stimnames, {'targXL'}))  .*  parallel.dim.cdfs.correct(:, mstrfind(parallel.dim.stimnames, {'targYH'})),... % LH
...
parallel.SIC = sic(parallel.S.correct(:, mstrfind(parallel.stimnames, {'LL', 'LH', 'HL', 'HH'})));
...
HH = fftConv(serial.dim.pdfs.correct(:,mstrfind(serial.dim.stimnames, {'targXH'})), serial.dim.pdfs.correct(:,mstrfind(serial.dim.stimnames, {'targYH'})));
HL = fftConv(serial.dim.pdfs.correct(:,mstrfind(serial.dim.stimnames, {'targXH'})), serial.dim.pdfs.correct(:,mstrfind(serial.dim.stimnames, {'targYL'})));
LH = fftConv(serial.dim.pdfs.correct(:,mstrfind(serial.dim.stimnames, {'targXL'})), serial.dim.pdfs.correct(:,mstrfind(serial.dim.stimnames, {'targYH'})));
...
% Need to convert individual dimension pdfs to pdfs for each item
serial.pdfs.correct = [HH, HL, LH, LL];
serial.cdfs.correct = cumsum(serial.pdfs.correct);
serial.S.correct = 1 - serial.cdfs.correct;
serial.SIC = sic(serial.S.correct(:, mstrfind(coactive.stimnames, {'LL', 'LH', 'HL', 'HH'})));
...
for p = 0:.2:1;
    ...
    mixedCoaPar.SIC(:,cnt) = sic(mixedCoaPar.S.correct(:,mstrfind(mixedCoaPar.stimnames, {'LL', 'LH', 'HL', 'HH'}), cnt));
    ...
    mixedSerPar.SIC(:,cnt)   = sic(mixedSerPar.S.correct(:,mstrfind(mixedSerPar.stimnames, {'LL', 'LH', 'HL', 'HH'}), cnt));
