...
settings.nGPUs    = 1;                       % Number of GPUs - if 0, use CPU
...
for i = 1
    ...
    plot(selection', mixedCoaPar.(fieldnames{i})(selection,:), '-')
    set(gca, 'XLim', xlims(i,:), 'YLim', ylims(i,:), 'FontSize', 10)
    xlabel('t (msec)', 'FontSize', 12)
    ylabel(labelnames{i}, 'FontSize', 12)
    title('Coactive/Parallel', 'FontSize', 14)
    legend(num2str((0:.2:1)'))
    
    subplot(1, 2, cnt); cnt = cnt + 1;
    plot(selection', mixedSerPar.(fieldnames{i})(selection,:), '-')
    set(gca, 'XLim', xlims(i,:), 'YLim', ylims(i,:), 'FontSize', 10)
    xlabel('t (msec)', 'FontSize', 12)
    ylabel(labelnames{i}, 'FontSize', 12)
    title('Serial/Parallel', 'FontSize', 14)
    legend(num2str((0:.2:1)'))
end
% % displayTime(toc)
toc
