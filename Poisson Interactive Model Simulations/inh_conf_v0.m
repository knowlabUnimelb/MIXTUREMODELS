...
T1_0  = diag([p00 * ones(c_d,1); p00_dc]) +...
         diag(p01 * ones(c_d,1), 1);              % Channel A at 0, B moves
T1_a0 = diag([p00 * ones(c_d,1); p00_dc]) +...
        diag((p01+p01_a0) * ones(c_d,1), 1);     % Channel A at intermediate counts, B moves
T1_ac = diag([p00_ac*ones(c_d,1);1]) +...
         diag(p01_ac*ones(c_d,1),1);              % Channel A has completed, B moves

T2_0  = diag([p1_1*ones(c_d-1,1);0], -1) + ...
        diag([p10+p10_d0; p10*ones(c_d-1,1);p10_dc]) + ...
        diag(p11 * ones(c_d,1), 1);
