...
%%
T1_a0 = diag([p00 * ones(c_b,1); p00+p01+p_11]) +...
         diag((p01+p_11) * ones(c_b,1), 1);        % Channel A at 0
T1_0  = diag([p00 * ones(c_b,1); p00+p01]) +...
         diag(p01 * ones(c_b,1), 1);              
T1_ac = diag([p00+p10+p1_1; (p00+p10)*ones(c_b-1,1); 1]) +...
        diag([p1_1 * ones(c_b-1,1); 0], -1) +...
        diag((p01+p11) * ones(c_b,1),1);          % Channel A has completed, B moves
    
T2_0  = diag(p1_1*ones(c_b,1), -1) + ...
        diag([p10+p1_1; p10*ones(c_b-1,1); p10+p11]) + ...
        diag(p11 * ones(c_b,1), 1);
    
T3_0  = diag(p_11 * ones(c_b,1), 1) +...
        diag([zeros(c_b,1); p_11]) ;
T3_ac = diag(p_11 * ones(c_b,1), 1);

T1 = mat2cell(repmat(T1_0, 1, c_a-1), c_b+1, repmat(c_b+1, 1, c_a-1));
T2 = mat2cell(repmat(T2_0, 1, c_a-1), c_b+1, repmat(c_b+1, 1, c_a-1));
T3 = mat2cell(repmat(T3_0, 1, c_a-1), c_b+1, repmat(c_b+1, 1, c_a-1));
...
T = blkdiag(T1_a0, T1{1:c_a-1}, T1_ac) + ...                        
    ...
    horzcat(blkdiag(vertcat(zeros(c_b+1,c_b+1), T3_0), T3{1:c_a-2}, T3_ac),...
          zeros((c_b+1)*(c_a+1), c_b+1));
